...
export function evaluateFrame(frame: AnalysisFrame): VoiceFeedback[] {
  ...
  if (frame.guard.overall < 40) {
    ...
    if (frame.guard.leftHandHeight < 60 || frame.guard.rightHandHeight < 60) {
      feedback.push({
        message: pickRandom(['Mãos mais altas, protege o rosto!', 'Mantém as mãos altas!', 'Guarda tá caindo um pouco!']),
        priority: 'high',
        category: 'guard',
        cooldownMs: 8000,
      });
    ...
    if (frame.guard.elbowTuck < 60) {
      feedback.push({
        message: pickRandom(['Cotovelos junto ao corpo!', 'Cola os cotovelos!', 'Cotovelos pra dentro!']),
        priority: 'high',
        category: 'guard',
        cooldownMs: 8000,
      });
  ...
  if (frame.guard.chinTuck < 50) {
    feedback.push({
      message: pickRandom(['Abaixa o queixo!', 'Protege o queixo!', 'Queixo pra baixo!']),
      priority: 'high',
      category: 'guard',
      cooldownMs: 10000,
    });
  ...
  if (frame.base.overall < 40) {
    feedback.push({
      message: pickRandom(['Corrige a base!', 'Ajusta a posição dos pés!', 'Base tá fraca!']),
      priority: 'critical',
      category: 'base',
      cooldownMs: 5000,
    });
    ...
    if (frame.base.footWidth < 60) {
      feedback.push({
        message: pickRandom(['Abre mais os pés!', 'Pés na largura dos ombros!', 'Afasta mais os pés!']),
        priority: 'normal',
        category: 'base',
        cooldownMs: 8000,
      });
    ...
    if (frame.base.kneeFlex < 60) {
      feedback.push({
        message: pickRandom(['Flexiona os joelhos!', 'Dobra mais os joelhos!', 'Não trava os joelhos!']),
        priority: 'normal',
        category: 'base',
        cooldownMs: 8000,
      });
  ...
  if (frame.base.weightDistribution < 50) {
    feedback.push({
      message: pickRandom(['Distribui o peso melhor!', 'Equilibra o peso entre os pés!', 'Peso mais equilibrado!']),
      priority: 'normal',
      category: 'base',
      cooldownMs: 10000,
    });
  ...
  if (frame.activePunch) {
    ...
    if (punch.quality === 'good') {
      feedback.push({
        message: pickRandom(['Bom golpe!', 'Mandou bem!', 'Golpe firme!', 'Isso aí!']),
        priority: 'low',
      ...
      feedback.push({
        message: pickRandom(['Retorna a mão mais rápido!', 'Traz a mão de volta!', 'Volta pra guarda mais rápido!']),
        priority: 'low',
        category: 'punch',
        cooldownMs: 6000,
        immediate: true,
      ...
      feedback.push({
        message: pickRandom(['Estende mais o braço!', 'Gira mais o quadril!', 'Mais extensão no braço!']),
        priority: 'normal',
        category: 'punch',
        cooldownMs: 6000,
        immediate: true,
  ...
  if (frame.guard.overall >= 90 && frame.base.overall >= 90) {
    feedback.push({
      message: pickRandom(['Postura excelente, continua assim!', 'Tá mandando bem!', 'Ritmo bom, continua!', 'Muito bem, mantém assim!']),
      priority: 'low',
      category: 'encouragement',
      cooldownMs: 15000,
    });
